...
const db = new sqlite3.Database("simetricdb.sqlite", (err) => {
  if (err) {
    ...
    console.log("Conectado a la base de datos para menú de personal.")
  }
})
...
function cargarEstadisticas() {
  ...
  db.get("SELECT COUNT(*) as total FROM empleados WHERE estatus = 'activo'", [], (err, row) => {
    if (err) {
      console.error("Error al obtener empleados activos:", err.message)
      return
    }
    document.getElementById("total-empleados").textContent = row.total || 0
  })

  // Nóminas pendientes del mes actual
  const fechaActual = new Date()
  const mesActual = fechaActual.getMonth() + 1
  const añoActual = fechaActual.getFullYear()

  db.get(
    "SELECT COUNT(*) as total FROM nomina WHERE mes = ? AND año = ? AND estatus = 'pendiente'",
    [mesActual, añoActual],
    (err, row) => {
      if (err) {
        console.error("Error al obtener nóminas pendientes:", err.message)
        return
      }
      document.getElementById("nomina-pendiente").textContent = row.total || 0
    },
  )
  ...
  db.get(
    ...
    (err, row) => {
      if (err) {
        console.error("Error al obtener total de nómina:", err.message)
        return
      }
      const total = row.total || 0
      document.getElementById("total-nomina").textContent = `$${total.toFixed(2)}`
    },
  )
}
...
function configurarRedireccion() {
  ...
  esAdministradorPrincipal(usuarioActual, (esGerente) => {
    if (esGerente) {
      ...
      backButton.href = "../dashboard/indexmain.html"
      ...
      backButton.href = "../dashboard/index.html"
...
document.addEventListener("DOMContentLoaded", () => {
  console.log("Configurando redirección para menú de personal...")
...
function mostrarInfoUsuario() {
  ...
  if (usuarioActual) {
    esAdministradorPrincipal(usuarioActual, (esGerente) => {
      ...
      console.log(`Redirección: ${esGerente ? "../dashboard/indexmain.html" : "../dashboard/index.html"}`)
    })
  } else {
